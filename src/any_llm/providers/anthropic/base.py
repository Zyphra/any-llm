...
class BaseAnthropicProvider(AnyLLM, ABC):
    ...
    async def _arequest(self, params: RequestParams, **kwargs: Any) -> RequestResponse:
        """Convert OpenResponses input to native Messages, then ResponseResource on the way back."""
        if params.stream:
            msg = "Streaming is not yet implemented for arequest; see docs/internal/arequest_streaming.md."
            raise NotImplementedError(msg)
        api_kwargs = _convert_request_params(params, self.PROVIDER_NAME, **kwargs)
        message = await self.client.messages.create(**api_kwargs)
        resource = _convert_request_response(message, params=params)
